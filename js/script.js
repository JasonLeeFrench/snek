...
var getFreeCoord = function getFreeCoord(obj, tileCount) {
  var grid = [].concat(_toConsumableArray(Array(tileCount ** 2).keys())).map(function (i) {
    return [Math.floor(i / tileCount), i % tileCount];
  });
  var fullTiles = obj.map(function (coords) {
    return Object.values(coords);
  });
  var freeTiles = grid.filter(function (tile) {
    return !fullTiles.some(function (coords) {
      return arraysAreSame(coords, tile);
    });
  });
  return freeTiles[Math.floor(Math.random() * freeTiles.length)];
};

var checkBounderies = function checkBounderies(directions, edge) {
  return directions.map(function (direction) {
    return direction = direction < 0 ? edge : direction > edge ? 0 : direction;
  });
};

var arraysAreSame = function arraysAreSame(a, b) {
...
var _Treat = __webpack_require__(331);
...
var elements = [player, treat];

var game = new _Game2.default({ elements: elements, ctx: ctx });
game.tick();
...
var Player = function () {
  ...
  _createClass(Player, [{
    ...
    key: 'update',
    value: function update(hitTreat) {
      var _this = this;
      ...
      var _checkBounderies = (0, _utils.checkBounderies)([this.x, this.y], _config.TILE_COUNT - 1);

      var _checkBounderies2 = _slicedToArray(_checkBounderies, 2);

      this.x = _checkBounderies2[0];
      this.y = _checkBounderies2[1];

      var touching = this.trail.some(function (bit) {
        return (0, _utils.isColliding)(bit, _this);
      });
      if (touching) {
        // boo, reset
        this.tail = 5;
    ...
    value: function draw(ctx) {
      ctx.fillStyle = _config.SNAKE_COLOR;
      this.trail.forEach(function (bit) {
        ctx.fillRect(bit.x * _config.GRID_SIZE, bit.y * _config.GRID_SIZE, _config.DRAW_AREA, _config.DRAW_AREA);
      });
    }
  }]);
...
var Treat = function () {
  ...
  _createClass(Treat, [{
    ...
    value: function draw(ctx) {
      ...
      ctx.fillStyle = _config.TREAT_COLOR;
      ctx.fillRect(x * _config.GRID_SIZE, y * _config.GRID_SIZE, _config.DRAW_AREA, _config.DRAW_AREA);
    }
  }]);
...
var Game = function () {
  ...
  _createClass(Game, [{
    ...
    value: function update() {
      var _elements = _slicedToArray(this.elements, 2),
          player = _elements[0],
          treat = _elements[1];
    ...
    value: function draw() {
      ...
      this.ctx.fillStyle = _config.CANVAS_COLOR;
